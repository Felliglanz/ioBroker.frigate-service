...
function createRuleEngine(adapter, config) {
  ...
  const translateLabel = label => {
    ...
    return translations[lbl] || (lbl ? lbl.charAt(0).toUpperCase() + lbl.slice(1) : 'Objekt');
  };
  ...
  async function handleFrigateEvent(ev) {
    ...
    for (const item of items) {
      ...
      const labelName = translateLabel(ev.label);
      const content = comment || `🔔 ${labelName} erkannt an ${cameraName}`;
      ...
      try {
        if (target.type === 'discordWebhook') {
          ...
          await sendDiscordWebhook({
            ...
            content,
            embed: {
              title: `${labelName} erkannt – ${cameraName}`,
              fields,
            },
            attachment,
          });
