...
function createRuleEngine(adapter, config) {
  ...
  const toDisplayName = cameraKey => {
    ...
    return found && found.name ? String(found.name) : cameraKey;
  };
          ...
          await sendDiscordWebhook({
            ...
            embed: {
              ...
              fields: [
                ...
                Array.isArray(ev.enteredZones) && ev.enteredZones.length ? { name: 'Entered zones', value: ev.enteredZones.map(z => String(z)).join(', '), inline: false } : null,
              ],
            },
            attachment,
          });
