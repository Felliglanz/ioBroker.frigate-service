...
function createRuleEngine(adapter, config) {
  ...
  const translateLabel = (label, lang) => {
    ...
    return dict[lbl] || (lbl ? lbl.charAt(0).toUpperCase() + lbl.slice(1) : (langKey === 'de' ? 'Objekt' : 'Object'));
  };
  ...
  async function handleFrigateEvent(ev) {
    ...
    for (const item of items) {
      ...
      const labels = Array.isArray(item.filter && item.filter.labels) ? item.filter.labels : [];
      if (labels.length && !labels.includes(ev.label)) continue;
      ...
      const lang = config.notificationLanguage || 'de';
      const labelName = translateLabel(ev.label, lang);
      const detectedText = lang === 'en' ? 'detected at' : 'erkannt an';
      const content = comment || `🔔 ${labelName} ${detectedText} ${cameraName}`;
      ...
      try {
        if (target.type === 'discordWebhook') {
          ...
          await sendDiscordWebhook({
            ...
            content,
            embed: {
              title: lang === 'en' ? `${labelName} detected – ${cameraName}` : `${labelName} erkannt – ${cameraName}`,
              fields,
            },
            attachment,
          });
