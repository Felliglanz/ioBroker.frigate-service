...
function extractCameraIdRoot(prefix, objectId) {
  const marker = `${prefix}.`;
  if (!objectId || !objectId.startsWith(marker)) return null;
  const rest = objectId.slice(marker.length);
  // Expect "<cameraId>" as direct child channel (no further dot)
...
async function discoverCamerasFromFrigateAdapter(adapter, frigateInstance) {
  ...
  // Heuristic 3: some Frigate adapters expose camera channels directly under frigate.X.<cameraId>
  // Example: frigate.0.einfahrt
  if (!discovered.length) {
    try {
      const channels = await adapter.getForeignObjectsAsync(`${prefix}.*`, 'channel');
      for (const [id, obj] of Object.entries(channels || {})) {
        const camId = extractCameraIdRoot(prefix, id);
        if (!camId || blacklist.has(camId)) continue;
  ...
  // Heuristic 4: some setups use device/folder objects for cameras
  if (!discovered.length) {
    for (const type of ['device', 'folder']) {
      try {
        const objs = await adapter.getForeignObjectsAsync(`${prefix}.*`, type);
  ...
  // Heuristic 5: infer camera id from any states like frigate.X.<cameraId>.<something>
  if (!discovered.length) {
    try {
      const states = await adapter.getForeignObjectsAsync(`${prefix}.*.*`, 'state');
      for (const id of Object.keys(states || {})) {
        const marker = `${prefix}.`;
        ...
        if (!camId || blacklist.has(camId)) continue;
        discovered.push({ id: String(camId), name: String(camId) });
      }
    } catch {
      // ignore
    }
